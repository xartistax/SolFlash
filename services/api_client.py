...
class APIClient:
    ...
    def get(self, url: str, params: dict = None):
        ...
        self._check_rate_limit()  # Check rate limit before making a request
    ...
    def _retry_request(self, url: str, retries: int, params: dict = None):
        ...
        while retries <= self.max_retries:
            try:
                ...
                logger.warning(f"Retry {retries}/{self.max_retries} for URL: {url}. Error: {e}")
                retries += 1
                if retries <= self.max_retries:
                    # Exponential backoff
                    sleep_time = min(2 ** retries, 30)  # Max sleep time capped at 30 seconds
                    time.sleep(sleep_time)
                else:
                    logger.error(f"Max retries exceeded for URL: {url}. Error: {e}")
                    return None, None
